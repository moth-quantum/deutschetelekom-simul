...
function startPythonStream() {
  ...
  pythonProcess = spawn('python', ['device_controller.py'], {
    env: { ...process.env, USE_REAL_HARDWARE: 'false' }
  });
  ...
  rl.on('line', (line) => {
    try {
      io.emit('numerical_data', JSON.parse(line));
    } catch (e) {
  ...
  socket.on('knobs', (data) => {
    if (currentMode.useRealHardware) {
      forwardToBridge(data, socket).catch((err) => {
        console.error('Unexpected bridge error:', err);
      });
